...
- **Retirar la mitad vanilla del golden-master harness** — `test_backend_matches_snapshot` sigue aportando valor real (regresión de toda la lógica financiera) y se mantiene. `test_frontend_matches_snapshot` + `run_frontend_harness.mjs` comparan contra `index.html` (vanilla, ya no servido ni tocado) y son redundantes: los `*.test.tsx` de React ya validan el frontend real contra el mismo fixture de backend. `build_fixture.py` además está roto desde el Bloque 1 y no es regenerable. Pendiente: decidir qué hacer con `index.html` congelado y actualizar §7/tests/README.md para dejar de describir esta suite como "temporal" -- en la práctica ya es la suite de regresión permanente del backend.
